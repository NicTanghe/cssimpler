...
fn main() -> Result<()> {
    ...
    let mut state = shared::EffectStressState::default();
    if shared::baseline_autostart_requested() {
        shared::start_baseline_harness(&mut state);
    }
...
fn fragments() -> Vec<Fragment<'static, shared::EffectStressState>> {
    let mut fragments = vec![
        Fragment::new("backdrop", |_state: &shared::EffectStressState| {
            shared::build_fragment_backdrop()
        }),
        Fragment::new("wall-shell", |_state: &shared::EffectStressState| {
            shared::build_fragment_wall_shell()
        }),
...
fn stylesheet() -> &'static Stylesheet {
    ...
    STYLESHEET.get_or_init(|| {
        ...
        parse_stylesheet(&css).expect("fragment pressure stylesheet should stay valid")
    })
}
