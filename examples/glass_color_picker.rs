...
fn main() -> Result<()> {
    let config = WindowConfig::new("cssimpler / glass color picker", 900, 560)
...
fn stylesheet() -> &'static Stylesheet {
    ...
    STYLESHEET.get_or_init(|| {
        parse_stylesheet(include_str!("glass_color_picker.css"))
            .expect("glass color picker example stylesheet should stay valid")
    })
}
...
mod tests {
    ...
    #[test]
    fn glass_color_picker_example_stylesheet_parses_and_builds_ui() {
        let _ = stylesheet();
    ...
    #[test]
    fn glass_color_picker_actions_update_state() {
