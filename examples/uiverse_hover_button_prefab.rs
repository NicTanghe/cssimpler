...
fn main() -> Result<()> {
    let config = WindowConfig::new("cssimpler / uiverse hover button / prefab", 1280, 720);

    FragmentApp::new((), stylesheet(), update, [Fragment::new("app", build_ui)])
        .run(config)
        .map_err(Into::into)
}

fn update(_state: &mut (), _frame: FrameInfo) -> Refresh {
    Refresh::clean()
}

fn build_ui(_state: &()) -> &'static cssimpler::core::StaticNodeDesc {
    ui_prefab! {
        <div id="app">
...
fn stylesheet() -> &'static Stylesheet {
    ...
    STYLESHEET.get_or_init(|| {
        ...
        parse_stylesheet(&source).expect("uiverse hover button prefab stylesheet should stay valid")
    })
}
