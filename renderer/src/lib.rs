...
struct CachedSubtreeSurface {
    ...
    width: usize,
    height: usize,
    // Cached surfaces use premultiplied linear RGBA so bilinear reconstruction
    // does not pull a black matte into transparent edges.
...
struct SubtreeSurfaceCache {
    ...
    builds: usize,
    total_bytes: usize,
    last_build_surface_bytes: usize,
    last_build_temp_bytes: usize,
...
fn insert_subtree_surface_entry(
    ...
    let byte_len = subtree_surface_byte_len(surface.as_ref());
    if byte_len > MAX_SUBTREE_SURFACE_CACHE_BYTES {
        return false;
    }
    while (!cache.surfaces.is_empty())
        && (cache.surfaces.len() >= MAX_SUBTREE_SURFACE_CACHE_ENTRIES
            || cache.total_bytes.saturating_add(byte_len) > MAX_SUBTREE_SURFACE_CACHE_BYTES)
    {
        evict_lru_subtree_surface_entry(cache);
    }
    if cache.total_bytes.saturating_add(byte_len) > MAX_SUBTREE_SURFACE_CACHE_BYTES {
        return false;
    }

    let last_used = next_surface_cache_use(&mut cache.next_use);
    cache.total_bytes = cache.total_bytes.saturating_add(byte_len);
    cache.surfaces.insert(
        key,
        SurfaceCacheEntry {
            surface,
            byte_len,
...
fn evict_lru_subtree_surface_entry(cache: &mut SubtreeSurfaceCache) {
    ...
    if let Some(entry) = cache.surfaces.remove(&oldest_key) {
...
fn sample_cached_surface_bilinear(
    ...
    // Promoted surfaces sample transparent texels beyond the cached rectangle,
    // but because the stored colors are premultiplied this does not inject a
    // matte fringe when bilinear taps cross the edge.
...
fn extract_surface_from_mattes(
    ...
    if width == 0
        || height == 0
        || black_buffer.len() < pixel_count
        || white_buffer.len() < pixel_count
    {
        return None;
    }
    ...
    for index in 0..pixel_count {
        ...
        let alpha = ((alpha_r + alpha_g + alpha_b) / 3.0).clamp(0.0, 1.0);
        ...
        pixels.push(LinearRgba {
            r: black.r.clamp(0.0, alpha),
            g: black.g.clamp(0.0, alpha),
            b: black.b.clamp(0.0, alpha),
            a: alpha,
        });
    }
...
fn surface_pixel_dimensions(
    ...
    (surface_width > 0 && surface_height > 0).then_some((surface_width, surface_height))
}

fn promoted_surface_storage_bytes(pixel_count: usize) -> usize {
    pixel_count.saturating_mul(size_of::<LinearRgba>())
}

fn promoted_surface_temp_bytes(pixel_count: usize) -> usize {
    pixel_count
        .saturating_mul(size_of::<u32>())
        .saturating_mul(2)
}

fn subtree_surface_byte_len(surface: &CachedSubtreeSurface) -> usize {
    promoted_surface_storage_bytes(surface.pixels.len())
}

fn translate_layout(layout: LayoutBox, dx: f32, dy: f32) -> LayoutBox {
    LayoutBox::new(layout.x + dx, layout.y + dy, layout.width, layout.height)
}

fn translated_render_subtree(node: &RenderNode, dx: f32, dy: f32) -> RenderNode {
    let mut node = node.clone();
    node.layout = translate_layout(node.layout, dx, dy);
    node.children = node
        .children
        .iter()
        .map(|child| translated_render_subtree(child, dx, dy))
        .collect();
    node
}
...
fn cached_promoted_surface(
    ...
    let (surface_width, surface_height) = surface_pixel_dimensions(source_bounds, width, height)?;
    let surface_pixel_count = surface_width.saturating_mul(surface_height);
    if surface_pixel_count == 0 {
        return None;
    }
    let surface_storage_bytes = promoted_surface_storage_bytes(surface_pixel_count);
    let temp_bytes = promoted_surface_temp_bytes(surface_pixel_count);
    if surface_storage_bytes > MAX_PROMOTED_SURFACE_BYTES
        || temp_bytes > MAX_PROMOTED_SURFACE_TEMP_BYTES
    {
        return None;
    }

    let local_surface_root =
        translated_render_subtree(&surface_root, -source_bounds.x0, -source_bounds.y0);
    let mut local_node_count = 0;
    let local_surface_bounds = cache_subtree_bounds(
        &local_surface_root,
        TransformMatrix3d::IDENTITY,
        None,
        false,
        &mut local_node_count,
    );
    let local_clip = ClipRect::full(surface_width as f32, surface_height as f32);
    let mut buffers = acquire_worker_buffers(&[surface_pixel_count, surface_pixel_count]);
    ...
    if !insert_subtree_surface_entry(&mut cache, key, surface.clone()) {
        return None;
    }
    Some(surface)
}
...
fn draw_cached_surface_transformed(
    ...
    for y in y0.max(row_start)..y1.min(row_end) {
        ...
        for x in x0..x1 {
            ...
            blend_premultiplied_linear_over(buffer, local_row_start + x as usize, sample);
        }
    }
}
...
fn clear_subtree_surface_cache_for_tests() {
    ...
    cache.builds = 0;
    cache.total_bytes = 0;
    cache.last_build_surface_bytes = 0;
    cache.last_build_temp_bytes = 0;
...
fn render_to_buffer_serial(
    ...
    with_render_alpha_target(alpha_buffer, || {
        ...
        for index in sorted_scene_indices(scene) {
            let node = &scene[index];
            let cached_bounds = &cached_bounds[index];
...
fn redraw_auto_scroll_indicator_regions(
    ...
    with_render_alpha_target(alpha_buffer, || {
        ...
        for bounds in bounds {
            ...
            let root_indices = root_indices_intersecting_clip(scene, &cached_bounds.roots, clip);
            draw_cached_root_indices(
                scene,
                &cached_bounds.roots,
                &root_indices,
...
fn root_indices_intersecting_clip(
    ...
    let mut indices = cached_bounds
        ...
        .collect::<Vec<_>>();
    sort_indices_by_stacking(scene, &mut indices);
    indices
}

fn sorted_scene_indices(scene: &[RenderNode]) -> Vec<usize> {
    let mut indices = (0..scene.len()).collect::<Vec<_>>();
    sort_indices_by_stacking(scene, &mut indices);
    indices
}

fn sort_indices_by_stacking(nodes: &[RenderNode], indices: &mut [usize]) {
    ...
    let mut entries = node
        ...
        .filter_map(|(index, child)| {
            ...
            Some(ProjectedChildEntry {
                ...
                stacking_level: stacking_context_level(child),
            })
        })
        .collect::<Vec<_>>();
...
#[derive(Clone)]
enum DeferredRenderClip {
    Rect(ClipRect),
    State(ClipState),
}

struct DeferredRenderStackingContext<'a> {
    node: &'a RenderNode,
    cached_bounds: &'a CachedSubtreeBounds,
    clip: DeferredRenderClip,
    parent_perspective: Option<PerspectiveContext>,
    in_3d_context: bool,
    level: i32,
    order: u64,
...
fn collect_deferred_render_stacking_contexts<'a>(
    node: &'a RenderNode,
    cached_bounds: &'a CachedSubtreeBounds,
    child_clip: DeferredRenderClip,
    parent_perspective: Option<PerspectiveContext>,
    in_3d_context: bool,
    order: &mut u64,
    deferred: &mut Vec<DeferredRenderStackingContext<'a>>,
) {
    ...
    for (index, child) in node.children.iter().enumerate() {
        ...
        if establishes_stacking_context(child) {
            deferred.push(DeferredRenderStackingContext {
                node: child,
                cached_bounds: child_bounds,
                clip: child_clip.clone(),
...
fn descendant_render_clip(
    parent_child_clip: &DeferredRenderClip,
    node: &RenderNode,
) -> Option<DeferredRenderClip> {
    if !node.style.overflow.clips_any_axis() {
        return Some(parent_child_clip.clone());
    }

    match parent_child_clip {
        DeferredRenderClip::Rect(clip) => {
            if node_has_rounded_overflow_clip(node) {
                ClipState::new(*clip)
                    ...
                    .map(DeferredRenderClip::State)
            } else {
                clip.intersect(layout_clip(node.layout))
                    .map(DeferredRenderClip::Rect)
            }
        }
        DeferredRenderClip::State(clip_state) => clip_state
            .push_layout_clip(
                node.layout,
                node.style.corner_radius,
                AffineTransform::IDENTITY,
...
fn sort_deferred_render_contexts(entries: &mut [DeferredRenderStackingContext<'_>]) {
    entries.sort_by(|left, right| {
        left.level
            .cmp(&right.level)
...
fn draw_deferred_render_context(
    entry: &DeferredRenderStackingContext<'_>,
    buffer: &mut [u32],
    width: usize,
    height: usize,
    clear_color: Color,
    ...
    match &entry.clip {
        DeferredRenderClip::Rect(clip) if !node_requires_projected_path(entry.node) => {
            draw_node_contents_internal(
                entry.node,
                entry.cached_bounds,
                buffer,
        ...
        DeferredRenderClip::Rect(clip) => draw_node_transformed_internal(
            entry.node,
            entry.cached_bounds,
            buffer,
            width,
        ...
        DeferredRenderClip::State(clip_state) => draw_node_transformed_internal(
            entry.node,
            entry.cached_bounds,
            buffer,
            width,
...
fn draw_node_contents_in_current_stacking_context(
    node: &RenderNode,
    cached_bounds: &CachedSubtreeBounds,
    buffer: &mut [u32],
    width: usize,
...
fn draw_node_contents_internal_impl(
    node: &RenderNode,
    cached_bounds: &CachedSubtreeBounds,
    buffer: &mut [u32],
    width: usize,
    ...
    if node_has_rounded_overflow_clip(node) {
        ...
        if draw_deferred_stacking_contexts {
            let mut order = 0;
            collect_deferred_render_stacking_contexts(
                node,
                cached_bounds,
                DeferredRenderClip::State(child_clip_state.clone()),
                parent_perspective,
                in_3d_context,
                &mut order,
                &mut deferred,
            );
            sort_deferred_render_contexts(&mut deferred);
        }

        for entry in deferred.iter().filter(|entry| entry.level < 0) {
            draw_deferred_render_context(
                entry,
                buffer,
                width,
                height,
    ...
    if draw_deferred_stacking_contexts {
        ...
        collect_deferred_render_stacking_contexts(
            node,
            cached_bounds,
            DeferredRenderClip::Rect(child_clip),
            parent_perspective,
            in_3d_context,
            &mut order,
            &mut deferred,
    ...
    for (index, child) in node.children.iter().enumerate() {
        ...
        let child_bounds = &cached_bounds.children[index];
        if !node_requires_projected_path(child) {
            draw_node_contents_in_current_stacking_context(
                child,
                child_bounds,
                buffer,
                width,
...
fn hit_test_node_for_event(
    ...
    contains.then(|| event_handler(node, event)).flatten()
}
...
struct DeferredHitStackingContext<'a> {
    node: &'a RenderNode,
    path: ElementPath,
    clip: ClipRect,
    parent_perspective: Option<PerspectiveContext>,
...
fn sort_deferred_hit_contexts(entries: &mut [DeferredHitStackingContext<'_>]) {
    entries.sort_by(|left, right| {
        left.level
            .cmp(&right.level)
            .then(left.order.cmp(&right.order))
...
fn hit_deferred_context_for_event(
    entry: &DeferredHitStackingContext<'_>,
    x: f32,
    y: f32,
    event: MouseEventKind,
) -> Option<EventHandler> {
    if !node_requires_projected_path(entry.node) {
        hit_test_node_for_event(
            entry.node,
            x,
            y,
...
fn hit_deferred_context_element_path(
    entry: &DeferredHitStackingContext<'_>,
    x: f32,
    y: f32,
) -> Option<ElementPath> {
...
fn hit_test_element_path_node(
    ...
    contains.then(|| node.element_path.clone().or_else(|| Some(path.clone())))?
}
...
fn hit_test_node_for_event_transformed(
    ...
    let contains = layout_contains(node.layout, local_x, local_y);
    if !contains && node.style.overflow.clips_any_axis() {
        return None;
    }
...
fn collect_node_dirty_regions(
    ...
    if node_requires_projected_path(previous) || node_requires_projected_path(current) {
        return collect_projected_node_dirty_regions(
            previous,
            previous_bounds,
            current,
            current_bounds,
...
fn collect_projected_node_dirty_regions(
    previous: &RenderNode,
    previous_bounds: &CachedSubtreeBounds,
    current: &RenderNode,
    current_bounds: &CachedSubtreeBounds,
    ...
    if !can_descend_projected_dirty_regions(previous, previous_bounds, current, current_bounds) {
        push_dirty_region(
            union_optional_bounds(previous_bounds.bounds, current_bounds.bounds),
            dirty_regions,
        );
    ...
    push_dirty_region(
        projected_leaf_dirty_region(previous, previous_bounds, current, current_bounds),
        dirty_regions,
    );

    if children_match && dirty_regions.len() == start_len {
        return true;
    }
...
fn can_descend_projected_dirty_regions(
    previous: &RenderNode,
    previous_bounds: &CachedSubtreeBounds,
    current: &RenderNode,
    current_bounds: &CachedSubtreeBounds,
) -> bool {
    render_nodes_match_own_visuals_ignoring_projection(previous, current)
        && projected_subtree_structure_is_stable(previous, current)
        && projected_projection_context_is_stable(previous, current)
        && !projected_branch_has_depth_ordering_risk(
            previous,
            previous_bounds,
            current,
            current_bounds,
...
fn projected_subtree_structure_is_stable(previous: &RenderNode, current: &RenderNode) -> bool {
    previous.children.len() == current.children.len()
}

fn projected_projection_context_is_stable(previous: &RenderNode, current: &RenderNode) -> bool {
    previous.layout == current.layout
        ...
        && previous.style.perspective == current.style.perspective
        && previous.style.transform_style == current.style.transform_style
        && (previous.style.transform == current.style.transform
            || (previous.children.is_empty() && current.children.is_empty()))
}

fn projected_branch_has_depth_ordering_risk(
    previous: &RenderNode,
    previous_bounds: &CachedSubtreeBounds,
    current: &RenderNode,
    current_bounds: &CachedSubtreeBounds,
) -> bool {
    previous.children.len().max(current.children.len()) > 1
        && (previous_bounds.subtree_uses_depth || current_bounds.subtree_uses_depth)
}

fn projected_leaf_dirty_region(
    previous: &RenderNode,
    previous_bounds: &CachedSubtreeBounds,
    current: &RenderNode,
    current_bounds: &CachedSubtreeBounds,
) -> Option<ClipRect> {
    if !previous.children.is_empty() || !current.children.is_empty() {
        return None;
    }

    if previous.style.transform == current.style.transform {
        return None;
    }
...
fn can_tighten_own_dirty_region(previous: &RenderNode, current: &RenderNode) -> bool {
    previous.kind == current.kind
        ...
        && previous.style.z_index == current.style.z_index
}
...
mod tests {
    ...
    use cssimpler_core::{
        ...
        TransformOperation, TransformStyleMode, VisualStyle, ZIndex,
    };
    ...
    #[test]
    fn positioned_z_index_paints_the_whole_subtree_above_later_siblings() {
        let menu_color = Color::rgb(34, 197, 94);
        let option_color = Color::rgb(37, 99, 235);
        let covered_color = Color::rgb(239, 68, 68);
        let scene = vec![
            RenderNode::container(LayoutBox::new(0.0, 0.0, 40.0, 40.0))
                ...
                .with_child(
                    RenderNode::container(LayoutBox::new(6.0, 6.0, 24.0, 24.0)).with_style(
                        VisualStyle {
                            background: Some(covered_color),
                            ..VisualStyle::default()
                        },
                    ),
                ),
        ];
        let mut buffer = vec![0_u32; 40 * 40];

        render_to_buffer(&scene, &mut buffer, 40, 40, Color::WHITE);

        assert_eq!(buffer[5 * 40 + 5], pack_rgb(menu_color));
        assert_eq!(buffer[10 * 40 + 10], pack_rgb(option_color));
    }

    #[test]
    fn nested_positioned_z_index_paints_above_later_uncle_without_lifting_parent() {
        let wrapper_color = Color::rgb(34, 197, 94);
        let menu_color = Color::rgb(37, 99, 235);
        let covered_color = Color::rgb(239, 68, 68);
        let scene = vec![
            RenderNode::container(LayoutBox::new(0.0, 0.0, 40.0, 40.0))
                .with_style(VisualStyle {
    ...
    #[test]
    fn non_positioned_z_index_does_not_reorder_siblings() {
        let early_color = Color::rgb(34, 197, 94);
        let later_color = Color::rgb(239, 68, 68);
        let scene = vec![
            RenderNode::container(LayoutBox::new(0.0, 0.0, 24.0, 24.0))
                .with_child(
                    RenderNode::container(LayoutBox::new(2.0, 2.0, 16.0, 16.0)).with_style(
                        VisualStyle {
                            background: Some(early_color),
                            z_index: ZIndex::Integer(1000),
                            ..VisualStyle::default()
                        },
                    ),
                ...
                .with_child(
                    RenderNode::container(LayoutBox::new(4.0, 4.0, 16.0, 16.0)).with_style(
                        VisualStyle {
                            background: Some(later_color),
                            ..VisualStyle::default()
                        },
                    ),
                ),
        ...
        assert_eq!(buffer[8 * 24 + 8], pack_rgb(later_color));
    }

    #[test]
    fn hit_testing_uses_positioned_z_index_order() {
        let root_path = ElementPath::root(0);
        let menu_path = root_path.with_child(0);
        let option_path = menu_path.with_child(0);
        let covered_path = root_path.with_child(1);
        let scene = vec![
            RenderNode::container(LayoutBox::new(0.0, 0.0, 40.0, 40.0))
                .with_element_path(root_path)
                .with_child(
                    RenderNode::container(LayoutBox::new(4.0, 4.0, 24.0, 24.0))
                        .with_element_path(menu_path)
                        .with_style(VisualStyle {
                            positioned: true,
                            z_index: ZIndex::Integer(1000),
                            ..VisualStyle::default()
                        ...
                        .with_child(
                            RenderNode::container(LayoutBox::new(8.0, 8.0, 8.0, 8.0))
                                .with_element_path(option_path.clone()),
                        ),
                )
                .with_child(
                    RenderNode::container(LayoutBox::new(6.0, 6.0, 24.0, 24.0))
                        .with_element_path(covered_path),
                ),
        ];

        let hit = hit_test_element_path(&scene, 10.0, 10.0);

        assert_eq!(hit, Some(option_path));
    }

    #[test]
    fn hit_testing_promotes_nested_positioned_z_index_without_lifting_parent() {
        let root_path = ElementPath::root(0);
        let wrapper_path = root_path.with_child(0);
        let menu_path = wrapper_path.with_child(0);
        let covered_path = root_path.with_child(1);
        let scene = vec![
            RenderNode::container(LayoutBox::new(0.0, 0.0, 40.0, 40.0))
                .with_element_path(root_path)
                .with_child(
                    RenderNode::container(LayoutBox::new(0.0, 0.0, 32.0, 32.0))
                        .with_element_path(wrapper_path)
                        .with_child(
                            RenderNode::container(LayoutBox::new(8.0, 8.0, 12.0, 12.0))
                                .with_element_path(menu_path.clone())
                                .with_style(VisualStyle {
                                    positioned: true,
                                    z_index: ZIndex::Integer(1000),
                                    ..VisualStyle::default()
        ...
        assert_eq!(hit_test_element_path(&scene, 10.0, 10.0), Some(menu_path));
        assert_eq!(hit_test_element_path(&scene, 5.0, 5.0), Some(covered_path));
    }
    ...
    #[test]
    fn hit_testing_reaches_visible_overflow_children() {
        CLICK_COUNT.store(0, Ordering::SeqCst);
        let scene = vec![
            RenderNode::container(LayoutBox::new(0.0, 0.0, 20.0, 20.0)).with_child(
                RenderNode::container(LayoutBox::new(4.0, 24.0, 48.0, 22.0))
                    .on_click(increment_click_count),
            ),
        ];

        assert!(dispatch_click(&scene, 12.0, 30.0));
        assert_eq!(CLICK_COUNT.load(Ordering::SeqCst), 1);
        assert_eq!(
            hit_test_element_path(&scene, 12.0, 30.0),
            Some(ElementPath {
                root: 0,
                children: vec![0],
            })
    ...
    #[test]
    fn promoted_surface_builds_use_subtree_sized_temp_buffers() {
        let _surface_cache_guard = super::lock_subtree_surface_cache_for_tests();
        super::clear_subtree_surface_cache_for_tests();
        let scene = promoted_surface_scene(
            Transform2D {
        ...
        let promoted = &scene[0].children[0];
        let surface_root = super::neutralized_surface_root(promoted);
        let mut node_count = 0;
        let surface_bounds = super::cache_subtree_bounds(
            &surface_root,
            TransformMatrix3d::IDENTITY,
            None,
            false,
        ...
        .expect("promoted subtree should stay inside the viewport");
        let (surface_width, surface_height) =
            super::surface_pixel_dimensions(source_bounds, 800, 600)
                .expect("promoted subtree should resolve to local surface pixels");

        let mut buffer = vec![0_u32; 800 * 600];
        render_to_buffer(&scene, &mut buffer, 800, 600, Color::WHITE);

        let expected_temp_bytes = surface_width
            .saturating_mul(surface_height)
            .saturating_mul(std::mem::size_of::<u32>())
            .saturating_mul(2);
        let expected_surface_bytes = surface_width
            .saturating_mul(surface_height)
            .saturating_mul(std::mem::size_of::<super::LinearRgba>());

        assert_eq!(super::subtree_surface_cache_builds_for_tests(), 1);
        assert_eq!(
            super::subtree_surface_last_build_temp_bytes_for_tests(),
            expected_temp_bytes
        );
        assert_eq!(
    ...
    #[test]
    fn oversized_promoted_surfaces_are_skipped_under_budget_pressure() {
        let _surface_cache_guard = super::lock_subtree_surface_cache_for_tests();
        super::clear_subtree_surface_cache_for_tests();
        let scene = vec![
            RenderNode::container(LayoutBox::new(0.0, 0.0, 800.0, 800.0))
                .with_style(VisualStyle {
                    background: Some(Color::rgb(245, 247, 250)),
                    ..VisualStyle::default()
                })
                .with_child(
                    RenderNode::container(LayoutBox::new(48.0, 40.0, 620.0, 620.0))
                        .with_style(VisualStyle {
                            background: Some(Color::rgb(40, 120, 220)),
                            corner_radius: CornerRadius::all(32.0),
                            transform: Transform2D {
                                operations: vec![TransformOperation::Translate {
                                    x: LengthPercentageValue::from_px(18.0),
                                    y: LengthPercentageValue::from_px(10.0),
                        ...
                        .with_child(
                            RenderNode::container(LayoutBox::new(120.0, 110.0, 240.0, 200.0))
                                .with_style(VisualStyle {
                                    background: Some(Color::rgb(220, 38, 38)),
        ...
        let mut buffer = vec![0_u32; 800 * 800];

        render_to_buffer(&scene, &mut buffer, 800, 800, Color::WHITE);
    ...
    #[test]
    fn premultiplied_surface_sampling_preserves_edge_color() {
        let surface = super::CachedSubtreeSurface {
            source_bounds: ClipRect {
                x0: 0.0,
                y0: 0.0,
                x1: 2.0,
            ...
            pixels: vec![
                ...
                super::LinearRgba::TRANSPARENT,
            ],
        };

        let sample = super::sample_cached_surface_bilinear(&surface, 1.0, 0.5);
        let mut buffer = vec![pack_rgb(Color::WHITE)];
        super::blend_premultiplied_linear_over(&mut buffer, 0, sample);
        let blended = super::unpack_rgb(buffer[0]).to_linear_rgba();

        assert!((sample.r - 0.5).abs() < 0.001);
        assert!((sample.a - 0.5).abs() < 0.001);
        assert!((blended.r - 1.0).abs() < 0.02);
        assert!((blended.g - 0.5).abs() < 0.05);
        assert!((blended.b - 0.5).abs() < 0.05);
    }
    ...
    #[test]
    fn projected_dirty_regions_descend_to_transform_only_leaf_updates() {
        let previous = vec![
            RenderNode::container(LayoutBox::new(0.0, 0.0, 420.0, 420.0))
                .with_style(VisualStyle {
                    background: Some(Color::rgb(28, 28, 30)),
                ...
                .with_child(
                    RenderNode::container(LayoutBox::new(60.0, 50.0, 290.0, 300.0))
                        .with_style(VisualStyle {
                            ...
                            corner_radius: CornerRadius::all(36.0),
                            perspective: Some(1000.0),
                            ..VisualStyle::default()
                        })
                        .with_child(
                            RenderNode::container(LayoutBox::new(98.0, 88.0, 170.0, 150.0))
                                .with_style(VisualStyle {
                                    background: Some(Color::rgb(0, 230, 150)),
                                    corner_radius: CornerRadius::all(24.0),
                                    transform_style: TransformStyleMode::Preserve3d,
                                    ..VisualStyle::default()
                                }),
                        ),
                        ...
                        .with_child(
                            RenderNode::container(LayoutBox::new(98.0, 88.0, 170.0, 150.0))
                                .with_style(VisualStyle {
                                    ...
                                    transform: Transform2D {
                                        operations: vec![
                                            TransformOperation::RotateX { degrees: 8.0 },
                                            TransformOperation::RotateY { degrees: -14.0 },
                                        ],
                                        ..Transform2D::default()
                                    },
                                    ..VisualStyle::default()
        ...
        let previous_bounds = super::cache_scene_subtree_bounds(&previous);
        let current_bounds = super::cache_scene_subtree_bounds(&next);
        let expected_leaf_bounds = super::union_optional_bounds(
            previous_bounds.roots[0].children[0].children[0].bounds,
            current_bounds.roots[0].children[0].children[0].bounds,
        )
        .expect("projected leaf should have bounds");
        let whole_branch_bounds = super::union_optional_bounds(
            previous_bounds.roots[0].children[0].bounds,
            current_bounds.roots[0].children[0].bounds,
        )
        .expect("projected ancestor should have bounds");

        assert_eq!(dirty_regions, vec![expected_leaf_bounds]);
        assert!(dirty_regions[0].area() < whole_branch_bounds.area());
    ...
    #[test]
    fn projected_dirty_regions_fallback_to_the_whole_branch_when_depth_order_can_change() {
        let previous = vec![
            RenderNode::container(LayoutBox::new(0.0, 0.0, 320.0, 240.0))
                .with_style(VisualStyle {
                    background: Some(Color::rgb(15, 23, 42)),
                    ..VisualStyle::default()
                })
                .with_child(
                    RenderNode::container(LayoutBox::new(40.0, 30.0, 220.0, 160.0))
                        .with_style(VisualStyle {
                            background: Some(Color::rgb(30, 41, 59)),
                            corner_radius: CornerRadius::all(28.0),
                            perspective: Some(900.0),
                            ..VisualStyle::default()
                        })
                        .with_child(
                            RenderNode::container(LayoutBox::new(72.0, 52.0, 120.0, 96.0))
                                .with_style(VisualStyle {
                        ...
                        .with_child(
                            RenderNode::container(LayoutBox::new(110.0, 74.0, 120.0, 96.0))
                                .with_style(VisualStyle {
                                    background: Some(Color::rgb(59, 130, 246)),
                                    corner_radius: CornerRadius::all(20.0),
                                    transform_style: TransformStyleMode::Preserve3d,
                                    transform: Transform2D {
                                        operations: vec![TransformOperation::TranslateZ {
                        ...
                        .with_child(
                            RenderNode::container(LayoutBox::new(110.0, 74.0, 120.0, 96.0))
                                .with_style(VisualStyle {
                                    ...
                                    transform_style: TransformStyleMode::Preserve3d,
                                    transform: Transform2D {
                                        operations: vec![TransformOperation::TranslateZ { z: 6.0 }],
                                        ..Transform2D::default()
                                    },
                                    ..VisualStyle::default()
                                }),
        ...
        let previous_bounds = super::cache_scene_subtree_bounds(&previous);
        let current_bounds = super::cache_scene_subtree_bounds(&next);
        let expected_branch_bounds = super::union_optional_bounds(
            previous_bounds.roots[0].children[0].bounds,
            current_bounds.roots[0].children[0].bounds,
        )
        .expect("projected ancestor should have bounds");

        assert_eq!(dirty_regions, vec![expected_branch_bounds]);
    }
